...
<head>
	...
	$(document).ready(function() {
		...
		$("#registerBtn").click(function() {
			let registerinfo = JSON.stringify({ // 제이슨 객체를 제이슨 문자열 모양으로 만듬.
		...
		$(document).on("dblclick", "tr.view", function() {
			...
			$.ajax({
				url:'${root}/admin/user/' + vid,  //여기서는 data : 로 json으로 전송한게 아니라 url에 포함해서 데이터 전송함.
				type:'GET',
				contentType:'application/json;charset=utf-8',
		...
		$(document).on("click", ".delBtn", function() {
			if(confirm("정말 삭제?")) {
				...
				$.ajax({
					...
					type:'DELETE',
					contentType:'application/json;charset=utf-8', //필요없음 보내는 데이터 url에 포함시키므로 // 얘를 없애려면 admincontroller에 headers부분 같이 지워야함.
					dataType:'json',
					success:function(users) {
						makeList(users);
					},
