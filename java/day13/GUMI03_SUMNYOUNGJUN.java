	...
	public static void main(String[] args) {
		...
		try {
			...
			while (socket != null) {
				...
				{
					angle = (float) ((float)Math.toDegrees(Math.atan2(height, width)) +90); // 스테이지2
				}
				...
				{
					angle = 89+ (float)((float)Math.toDegrees(Math.atan2(height, width))); // 스테이지2
				}
				if(targetBall_y < whiteBall_y && targetBall_x < whiteBall_x) {
					angle = 179+ (90-(float)((float)Math.toDegrees(Math.atan2(height, width)))); // 스테이지2
				}
